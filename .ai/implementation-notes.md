...
`angular.json` defines `build`, `serve`, and `deploy` targets only. There is no `test` target and no spec files, so `npm test` fails. `npm run build` generates version metadata and builds `production-local`; `npm run build:gh` targets `production-gh`. The ignored `core/generated/app-version.ts` is generated by postinstall and npm start/build scripts. Before direct `ng` commands, run `npm run version:generate`. Working commands are in `web/cheatsheet.txt`.